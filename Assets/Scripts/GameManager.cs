...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (m_MoveInput.x != 0)
        {
            int multiplier = m_MoveInput.x > 0 ? 1 : -1;
            pivots[0].DOComplete();
            pivots[0]
                .DORotate(new Vector3(0, 90 * multiplier, 0), .6f, RotateMode.WorldAxisAdd)
        ...
        foreach (Transform t in objectsToHide)
        {
            t.gameObject.SetActive(
                pivots[0].eulerAngles.y > 45 && pivots[0].eulerAngles.y < 90 + 45
            );
        }
